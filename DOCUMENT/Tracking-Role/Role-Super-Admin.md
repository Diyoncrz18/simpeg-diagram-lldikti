...
| 5 | P1 | **Data Master — sebagian besar ditutup PR #134 (merged 27 Juli)**: CRUD lima tabel referensi (golongan, jenis jabatan, eselon, status pegawai, jenjang pendidikan) + halaman Data Master berbasis database + guard baris sistem, sesuai keputusan K-1. Sisa: tab referensi lain (unit kerja hierarkis, jenis cuti, agama, status perkawinan, BUP, hari libur), UI channel & event notifikasi, keputusan `ref_bup` | Task tracker #22 (Issue #46) — lanjutan slice |
...
| ❌ Data Master statis | ⚠️ Sebagian besar ditutup PR #134 (merged 27 Juli) — sisa tab lanjutan, channel notifikasi, dan keputusan `ref_bup` |
