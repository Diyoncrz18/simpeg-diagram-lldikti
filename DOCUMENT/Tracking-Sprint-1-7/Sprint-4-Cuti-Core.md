...
| Pembaruan terakhir | 27 Juli 2026 (PR #135–#137 CI hijau, menunggu review) |
...
| #30 | Form pengajuan cuti | ⚠️ | Form, lampiran tervalidasi, hitung server-side, saldo, snapshot chain, notifikasi, audit, test tersedia. Dropdown jenis cuti kini menyaring PNS/PPPK dari metadata `khusus_pns` (**ditutup PR #104**) dengan validasi server sebagai lapis kedua. **Gap tersisa:** validasi lintas tahun kalender hanya berlaku untuk cuti yang memotong saldo tahunan (cek bersarang di `validateSaldoTahunan()` pada Store & Resubmit request) — Cuti Sakit/Melahirkan/Alasan Penting/Besar/CLTN masih bisa diajukan Des–Jan, padahal PRD §9.4 melarang untuk semua jenis. **Progres 27 Juli — PR #136 (CI hijau, menunggu review):** cek tahun diangkat menjadi validasi mandiri untuk SEMUA jenis pada Store & Resubmit (dijalankan sebelum gate saldo & cek TMT), pesan generik; +7 skenario test dan test E2E alur penuh submit→Kabag→PYBMC→potong saldo (`CutiEndToEndApprovalTest`). |
...
1. **P1 — Validasi lintas tahun untuk semua jenis cuti** (#30): angkat cek tahun keluar dari `validateSaldoTahunan()` di `StoreLeaveRequestRequest` & `ResubmitLeaveRequestRequest`; test lintas tahun untuk jenis non-tahunan. **→ Dikerjakan: PR #136 menunggu review.**
...
| 27 Juli 2026 | Perbaikan A dieksekusi: **PR #136** (validasi lintas tahun semua jenis — gap #30) + test E2E alur pengajuan→approval→potong saldo. Pendukung: **PR #135** (penetapan Kepala Bagian inline + penataan ulang halaman konfigurasi approval + pencarian PYBMC mandiri — memperhalus alur #27/#28) dan **PR #137** (dokumen bukti tersimpan kini memakai template formulir resmi yang sama dengan halaman pegawai — penyempurnaan #31). Ketiganya CI hijau, menunggu review. |
